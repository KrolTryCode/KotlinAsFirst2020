...
fun containsIn(a: Map<String, String>, b: Map<String, String>): Boolean {
    ...
    for ((key, value) in a)
        when{
            value == b[key] -> result = true
            value != b[key] -> result = false
        }
return result
}
/**
...
fun subtractOf(a: MutableMap<String, String>, b: Map<String, String>): MutableMap<String, String> {
    ...
    for ((key, value) in a) {
        if (b[key] == value && key in b) removeSet.add(key)
    }
    for (key in removeSet) a.remove(key)
    return a
}
...
fun findCheapestStuff(stuff: Map<String, Pair<String, Double>>, kind: String): String? {
  ...
  var name = ""
  for((value) in stuff){
      if(stuff[value]?.first == kind && stuff[value]?.second!! < price  ){
              price = stuff[value]?.second ?: return null
              name = value
  }
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list.indices) {
        ...
        if (addition !in map.keys)
            map.put(list[i], i)
        else return Pair(minOf(map[addition] ?: 0, i), maxOf(map[addition] ?: 0, i))
    }
    return Pair(-1, -1)
}
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    val ves = mutableListOf<Int>()
    val a = Array(treasures.size + 1) { Array(capacity + 1 ){ 0 } }
    val cen = mutableListOf<Int>()
    ...
    for((key , value) in treasures){
        name.add(key)
        ves.add(value.first)
        cen.add(value.second)

    }
    for(i in 1 .. treasures.size){
        for(j in 0 .. capacity) {
            if (j >= ves[i - 1])
                a[i][j] = maxOf(a[i - 1][j], a[i - 1][j - ves[i - 1]] + cen[i - 1])
